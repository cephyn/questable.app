...
def find_matching_standard_system(game_system_name):
    ...
    exact_match_query = standard_systems.where('standardName', '==', game_system_name).limit(1)
    ...
    for system in all_systems:
        ...
        if normalize_game_system_name(system_data.get('standardName')) == normalized_name:
            return {
                'id': system.id,
    ...
    for system in all_systems:
        ...
        if normalized_name and normalized_standard:
            if normalized_name in normalized_standard or normalized_standard in normalized_name:
...
def record_system_mapping_metrics(match_result, original_system):
    ...
    system_metrics_ref = metrics_ref.collection('systems').document(match_result['standardName'])
...
@firestore_fn.on_document_created(document='questCards/{questId}')
def standardize_new_quest_card(event: firestore_fn.Event[firestore_fn.DocumentSnapshot]) -> None:
    ...
    try:
        ...
        if 'standardizedGameSystem' in quest_data and quest_data['standardizedGameSystem']:
            return
...
@firestore_fn.on_document_updated(document='questCards/{questId}')
def handle_quest_card_update(event: firestore_fn.Event[firestore_fn.Change[firestore_fn.DocumentSnapshot]]) -> None:
    ...
    try:
        ...
        if ('gameSystem' not in before_data or 
            'gameSystem' not in after_data or 
            before_data.get('gameSystem') == after_data.get('gameSystem')):
            return
...
def scheduled_game_system_cleanup(event: scheduler_fn.ScheduledEvent) -> None:
    ...
    try:
        ...
        migration_ref = firestore.client().collection('migration_logs').document(migration_id)
        migration_ref.set({
            'timestamp': firestore.SERVER_TIMESTAMP,
            'type': 'scheduled',
            'status': 'in_progress'
        ...
        query = firestore.client().collection('questCards')\
        ...
        today = datetime.datetime.now().strftime('%Y-%m-%d')
        stats = get_standardization_stats()
        
        report_ref = firestore.client().collection('standardization_reports').document(today)
        report_ref.set({
            'date': today,
            'timestamp': firestore.SERVER_TIMESTAMP,
            'stats': stats,
...
def get_standardization_stats(req: https_fn.CallableRequest = None) -> dict:
    ...
    try:
        ...
        standardized_count = db.collection('questCards')\
            .where('systemMigrationStatus', '==', 'completed')\
            .count().get()[0].count
            
        pending_count = db.collection('questCards')\
            .where('systemMigrationStatus', '==', 'pending')\
            .count().get()[0].count
            
        failed_count = db.collection('questCards')\
            .where('systemMigrationStatus', '==', 'failed')\
            .count().get()[0].count
            
        needs_review_count = db.collection('questCards')\
            .where('systemMigrationStatus', '==', 'needs_review')\
            .count().get()[0].count
            
        no_match_count = db.collection('questCards')\
        ...
        unprocessed_count = total_count - (standardized_count + pending_count + failed_count + needs_review_count + no_match_count)
...
def process_system_mapping_feedback(event: firestore_fn.Event[firestore_fn.DocumentSnapshot]) -> None:
    ...
    try:
        ...
        standard_system_query = db.collection('game_systems')\
        ...
        query = db.collection('questCards')\
